...
([01](01-llm-foundations.md)) should not act **without oversight** when the stakes are high.
...
| Form | Description |
|------|-------------|
| **Action approval (gating)** | The agent proposes a risky tool call; execution is blocked until a human confirms. See [03 — Tools](03-tools-function-calling.md). |
| **Output review** | The agent produces a result a human must approve before publication. |
| **Human chat manager** | In a *group chat* orchestration ([08](08-multi-agent-orchestration.md)), a person can take the coordinator role, guiding the discussion among agents. |
...
HITL is one of the **guardrails** discussed in the [security](12-security-governance.md) chapter: it is
...
Previous: [10 — Evaluation and observability](10-evaluation-observability.md) · Next:
[12 — Security and governance](12-security-governance.md).
